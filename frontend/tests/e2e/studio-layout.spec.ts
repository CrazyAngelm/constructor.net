...
test('resize reflow, two columns, typography, personal folders and text previews survive reopen and print', async ({ page, browser }, testInfo) => {
	...
	try {
		...
		let library = page.getByRole('dialog', { name: 'Мои конспекты' })
		await library.locator('article').filter({ hasText: saved.name }).getByRole('button', { name: 'Открыть', exact: true }).click()
		...
		await page.getByRole('button', { name: 'Мои конспекты', exact: true }).click()
		library = page.getByRole('dialog', { name: 'Мои конспекты' })
		await library.getByLabel('Новая папка').fill(folderName)
		...
		const savedCard = library.locator('article').filter({ hasText: saved.name })
		...
		await library.getByRole('button', { name: 'Сохранить название' }).click()
		await expect(library.getByRole('heading', { name: folderName + ' — изменено', exact: true })).toBeVisible()
		await library.getByRole('button', { name: 'Удалить папку', exact: true }).click()
		await library.getByRole('button', { name: 'Удалить папку', exact: true }).click()
		await expect(library.locator('article').filter({ hasText: saved.name })).toBeVisible()
		await library.getByRole('button', { name: 'Закрыть мои конспекты' }).click()

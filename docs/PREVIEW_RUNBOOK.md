...
Для текущего сервера без Docker применяется отдельный native-контур: [PREVIEW_NATIVE.md](PREVIEW_NATIVE.md). Описанный ниже Compose-вариант остаётся альтернативой для Docker-хоста.
